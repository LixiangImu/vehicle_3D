...
class VehicleDetector3D:
    ...
    def visualize_3d_scene(self, detections):
        ...
        vis.create_window(width=1280, height=720)  # 设置固定的窗口大小
        ...
        # 设置一个俯视的相机视角（模拟从3楼向下拍摄的视角）
        camera_distance = 200  # 增加相机到场景中心的距离
        camera_height = 100   # 相机高度
        camera_x_offset = 0   # X轴偏移量
        
        # 设置相机位置和朝向
        # 相机位置：X=0（红轴），Y为正（绿轴上方），Z为负（蓝轴后方）
        camera_pos = np.array([camera_x_offset, camera_height, -camera_distance])
        look_at = np.array([0, 0, 0])  # 看向原点
        up = np.array([0, 1, 0])  # 上方向保持不变
        
        # 计算front向量（从相机指向目标的方向）
        front = (look_at - camera_pos) / np.linalg.norm(look_at - camera_pos)
        
        # 设置视角参数
        ctr.set_front(front)
        ctr.set_lookat(look_at)
        ctr.set_up(up)
        ctr.set_zoom(0.07)  # 减小zoom值以获得更广的视野
