...
class VehicleDetector3D:
    ...
    def image_to_world_coordinates(self, image_point):
        ...
        # 假设相机高度为5米，倾斜30度朝向地面
        camera_height = 5.0
        camera_tilt = np.radians(30)  # 相机俯仰角（30度）
        
        # 计算射线与地平面的交点
        # 考虑相机倾斜角度
        tan_tilt = np.tan(camera_tilt)
        scale = camera_height / (y * np.cos(camera_tilt) + tan_tilt + 1e-6)
        
        # 计算世界坐标
        world_x = x * scale
        world_z = scale * np.cos(camera_tilt)
        world_y = 0  # 地平面高度
    ...
    def estimate_vehicle_pose(self, front_point, rear_point, bbox):
        ...
        distance_factor = 1000 / (image_height + 1e-6)  # 使用图像高度估计距离
        scale_factor *= distance_factor / 100  # 调整系数
    ...
    def visualize_3d_scene(self, detections):
        ...
        ctr.set_zoom(0.3)
        # 设置相机位置和朝向以匹配实际图片
        ctr.set_front([-0.5, -0.8, -0.3])  # 调整相机方向
        ctr.set_lookat([0, 0, 0])          # 观察点
        ctr.set_up([0, 1, 0])              # 向上方向
