...
class VehicleDetector3D:
    ...
    def visualize_3d_scene(self, detections):
        ...
        # 设置一个俯视的相机视角
        camera_distance_z = -200  # Z轴距离（蓝轴负方向）
        camera_height = 100     # Y轴高度（绿轴正方向）
        camera_offset_x = 0     # X轴偏移（红轴）
        
        # 设置相机位置和朝向
        # X: 红轴，Y: 绿轴（上），Z: 蓝轴
        camera_pos = np.array([camera_offset_x, camera_height, -camera_distance_z])  
        look_at = np.array([0, 0, 0])  # 看向原点
        up = np.array([0, 1, 0])       # Y轴（绿轴）向上
        ...
        ctr.set_up(up)
        ctr.set_zoom(0.1)  # 保持较小的zoom值以获得宽广视野
