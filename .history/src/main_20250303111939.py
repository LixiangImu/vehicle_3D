...
def create_side_by_side_view(detection_img, render_img, target_size=1280):
    """创建并排显示的视图"""
    # 创建白色背景
    combined = np.ones((target_size, target_size * 2, 3), dtype=np.uint8) * 255
    
    # 调整两张图片的大小以适应目标尺寸
    detection_resized = cv2.resize(detection_img, (target_size, target_size))
    render_resized = cv2.resize(render_img, (target_size, target_size))
    
    # 将两张图片并排放置
    combined[:, :target_size] = detection_resized
    combined[:, target_size:] = render_resized
    
    # 添加分割线
    cv2.line(combined, (target_size, 0), (target_size, target_size), (0, 0, 0), 2)
...
def main():
    ...
    # 分别保存结果
    cv2.imwrite("detection_result.jpg", detection_vis)
    cv2.imwrite("3d_render_result.jpg", render_result)
    cv2.imwrite("combined_result.jpg", combined_view)
