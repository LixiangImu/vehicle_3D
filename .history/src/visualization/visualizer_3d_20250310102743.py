...
class Visualizer3D:
    ...
    def _setup_camera(self, vis: o3d.visualization.Visualizer) -> None:
        ...
        try:
            ...
            # 设置相机视角参数
            camera_height = self.config['camera']['default_height']  # 相机高度，通常在1.2~1.5米
            camera_pitch = -15  # 相机俯仰角，通常为-15度左右
            
            # 计算相机位置和朝向
            pitch_rad = np.deg2rad(camera_pitch)
            camera_pos = np.array([0, camera_height, 0])  # 相机位置
            
            # 计算相机旋转矩阵（绕X轴旋转pitch角度）
            Rx = np.array([
                [1, 0, 0],
            ...
            # 设置相机参数
            front = np.dot(Rx, np.array([0, 0, 1]))  # 相机朝向
            up = np.dot(Rx, np.array([0, 1, 0]))     # 相机上方向
            
            # 设置视角
            ctr.set_front(front)
            ctr.set_lookat([0, 0, 10])  # 看向前方10米处
            ctr.set_up(up)
            
            # 设置投影参数
            vis.get_view_control().convert_from_pinhole_camera_parameters(
                o3d.camera.PinholeCameraParameters(
                    extrinsic=np.eye(4),  # 使用单位矩阵作为外参
