...
class VehicleDetector3D:
    ...
    def image_to_world_coordinates(self, image_point, depth_factor=1.0):
        ...
        camera_tilt = np.radians(-30)  # 相机俯视角度（负值表示向下看）
        
        # 考虑图像y坐标来调整深度
        depth = camera_height * depth_factor * (1 + y)  # y越大（越靠下），深度越大
        
        # 计算世界坐标（考虑相机俯视角度）
        world_x = x * depth
        world_z = depth * np.cos(camera_tilt)
        world_y = 0  # 确保在地平面上
    ...
    def estimate_vehicle_pose(self, front_point, rear_point, bbox):
        ...
        depth_factor = 1.0 + (y_center / image_height)  # 越靠下深度因子越大
        ...
        center_3d[1] = 0  # 确保中心点在地面上
        
        # 根据边界框大小估计车辆尺寸
        bbox_height = bbox[3] - bbox[1]
        scale_factor = 0.1 * (1000 / (bbox_height + 1e-6))  # 根据边界框高度调整尺寸
