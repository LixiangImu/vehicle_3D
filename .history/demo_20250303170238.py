...
class VehicleDetector3D:
    ...
    def visualize_3d_scene(self, detections):
        ...
        # 设置相机视角（使用你调整后获得的实际参数）
        ctr = vis.get_view_control()
        # 设置相机参数（这些值需要替换为你调整后的实际值）
        ctr.set_zoom(0.1)
        ctr.set_front([-0.49, -0.87, 0.06])  # 示例值，需要替换
        ctr.set_lookat([10.0, 0.0, 0.0])     # 示例值，需要替换
        ctr.set_up([0.0, -0.11, 0.99])       # 示例值，需要替换
        
        # 当你调整好视角后，在渲染窗口中按 P 键
        # 控制台会打印出实际的相机参数
        # 把打印出的参数复制到这里即可
