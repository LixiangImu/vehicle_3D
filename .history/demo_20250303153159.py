...
class VehicleDetector3D:
    ...
    def detect_vehicles(self, image):
        ...
        """
        # 加载YOLO模型
        model = torch.hub.load('ultralytics/yolov5', 'custom', path='post.pt')
        ...
        for det in results.xyxy[0]:  # 遍历每个检测结果
            if det is not None and len(det) > 0:
                ...
                # 获取关键点坐标（假设关键点在det的后面）
                # 注意：这里需要根据您的模型输出格式进行调整
                keypoints = det[6:].cpu().numpy()  # 关键点坐标
                front_point = keypoints[:2]  # 前端关键点 [x, y]
                rear_point = keypoints[2:4]  # 后端关键点 [x, y]
    ...
    def image_to_world_coordinates(self, image_point):
        ...
        将图像坐标转换为世界坐标
        """
        # 将输入点转换为正确的格式：float32类型的 1x1x2 数组
    ...
    def visualize_3d_scene(self, detections):
        ...
        for detection in detections:
            ...
            # 修改这部分：将center=True改为具体的中心点坐标
            R = car_model_transformed.get_rotation_matrix_from_xyz((0, angle, 0))
            center_point = car_model_transformed.get_center()
            car_model_transformed.rotate(R, center=center_point)
            car_model_transformed.scale(scale, center=center_point)
